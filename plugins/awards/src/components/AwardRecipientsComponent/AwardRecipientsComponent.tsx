...
export const AwardRecipientsCard = ({
  recipients,
}: AwardRecipientsCardProps) => {
  return (
      ...
      <Typography variant="body1">
        <Grid container spacing={3}>
          {(recipients || []).map(recipient => {
            const displayName =
              recipient.profile?.displayName ?? recipient.metadata.name;
            const email = recipient.profile?.email
              ...
              <Grid item>
                <Box alignItems="center" display="flex" flexDirection="column">
                  <Avatar
                    ...
                    picture={recipient.profile?.picture}
                  />
