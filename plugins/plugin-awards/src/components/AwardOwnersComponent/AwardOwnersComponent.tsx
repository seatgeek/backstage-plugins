...
export const AwardOwnersCard = ({ recipients: owners }: AwardOwnersCardProps) => {
  ...
  return (
    ...
    <Typography variant="body1">
      ...
      {(owners || []).map(owner => {
        const displayName = owner.profile?.displayName ?? owner.metadata.name;
        const email = owner.profile?.email ? `(${owner.profile.email})` : '';
        ...
        return (
          <Grid item>
            <Box alignItems="center" display="flex" flexDirection="column">
              <Avatar displayName={displayName} picture={owner.profile?.picture} />
...
export const AwardOwnersComponent = ({uid}: AwardOwnersComponentProps) => {
  ...
  const { value, loading, error } = useAsync(async (): Promise<any[]> => {
    ...
    if (res.length > 0) {
      ...
      let ents = await catalogApi.getEntitiesByRefs({entityRefs: award.owners});
      return ents.items;
    } else {
