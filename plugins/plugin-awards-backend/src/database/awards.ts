...
const migrationsDir = resolvePackagePath(
  ...
  '@internal/plugin-awards-backend', // Package name
...
export class DatabaseAwardsStore implements AwardsStore {
  ...
  async search(
    ...
    let query = this.db<Award>('awards')
      ...
      .leftJoin('awards_recipients', 'awards.uid', 'awards_recipients.awards_uid')
      .groupBy('awards.uid')
      .orderBy('awards.uid')
      .select(
        this.db.raw(`${aggFunctionBegin}distinct awards_owners.owner_ref${aggFunctionEnd} as owners`),
        this.db.raw(`${aggFunctionBegin}distinct awards_recipients.recipient_ref${aggFunctionEnd} as recipients`),
      );
      ...
      return query.then((res) => {
        ...
        return res.map((awd) => {
          ...
          awd.recipients = isEmpty(awd.recipients) ? [] : awd.recipients.split(',');
          return awd;
        })
      })
  }
  ...
  async add(
    ...
    try {
      await this.db.transaction(async (tx) => {
        ...
        await tx<Award>('awards')
          .insert(record)
          .onConflict(['uid'])
          .merge();

        
        await tx.table('awards_owners')
          .insert(ownersToAdd.map((owner) => ({ 
            awards_uid: record.uid, 
            owner_ref: owner ,
          })));

        if (recipientsToAdd.length > 0) {
          await tx.table('awards_recipients')
            .insert(recipientsToAdd.map((member) => ({ 
              awards_uid: record.uid, 
    ...
    return new Promise<Award>((resolve) => resolve({
      ...record,
      owners: ownersToAdd,
  ...
  async update(
    ...
    try {
      await this.db.transaction(async (tx) => {  
        ...
        const currentOwners: string[] = await tx.table("awards_owners")
          .where({awards_uid: uid})
          .pluck("owner_ref");
        const ownersToAdd = owners.filter(owner => !currentOwners.includes(owner) && !isEmpty(owner));
        const ownersToRemove = currentOwners.filter(owner => !owners.includes(owner) && !isEmpty(owner));
      
        const currentRecipients: string[] = await tx.table("awards_recipients")
          .where('awards_uid', uid)
          .pluck("recipient_ref");
        const recipientsToAdd = recipients.filter(member => !currentRecipients.includes(member) && !isEmpty(member));
        const recipientsToRemove = currentRecipients.filter(member => !recipients.includes(member) && !isEmpty(member));
        ...
        // Refresh from db
        resOwners = await tx.table("awards_owners")
          .where({awards_uid: uid})
          .pluck("owner_ref");
        resRecipients = await tx.table("awards_recipients")
          .where('awards_uid', uid)
          .pluck("recipient_ref");
  ...
  async delete(
    ...
    const res = await this.db<Award>('awards')
      .where({uid: uid})
      .delete();
